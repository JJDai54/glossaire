...
namespace XoopsModules\Glossaire\Common;
...
class DirectoryChecker
{
    ...
    public static function getDirectoryStatus($path, $mode = 0777, $redirectFile = null)
    {
        ...
        if (!@\is_dir($path)) {
            ...
            $path_status .= "$path (" . \constant('_GLOSSAIRE_DC_NOTAVAILABLE') . ') ';
            ...
            $path_status .= "<button class='submit' onClick='this.form.submit();'>" . \constant('_GLOSSAIRE_DC_CREATETHEDIR') . '</button>';
            $path_status .= '</form>';
        } elseif (@\is_writable($path)) {
            $path_status = "<img src='$pathIcon16/1.png' >";
            $path_status .= "$path (" . \constant('_GLOSSAIRE_DC_AVAILABLE') . ') ';
            $currentMode = \mb_substr(\decoct(\fileperms($path)), 2);
            if ($currentMode != \decoct($mode)) {
                $path_status = "<img src='$pathIcon16/0.png' >";
                $path_status .= $path . \sprintf(\constant('_GLOSSAIRE_DC_NOTWRITABLE'), \decoct($mode), $currentMode);
                $path_status .= "<form action='" . $_SERVER['SCRIPT_NAME'] . "' method='post'>";
                $path_status .= "<input type='hidden' name='op' value='setdirperm'>";
                $path_status .= "<input type='hidden' name='mode' value='$mode'>";
                $path_status .= "<input type='hidden' name='path' value='$path'>";
                $path_status .= "<input type='hidden' name='redirect' value='$redirectFile'>";
                $path_status .= "<button class='submit' onClick='this.form.submit();'>" . \constant('_GLOSSAIRE_DC_SETMPERM') . '</button>';
                $path_status .= '</form>';
            }
        } else {
            $currentMode = \mb_substr(\decoct(\fileperms($path)), 2);
...
switch ($op) {
    case 'createdir':
        ...
        $msg = DirectoryChecker::createDirectory($path) ? \constant('_GLOSSAIRE_DC_DIRCREATED') : \constant('_GLOSSAIRE_DC_DIRNOTCREATED');
    ...
    case 'setdirperm':
        ...
        $msg = DirectoryChecker::setDirectoryPermissions($path, $mode) ? \constant('_GLOSSAIRE_DC_PERMSET') : \constant('_GLOSSAIRE_DC_PERMNOTSET');
        \redirect_header($redirect, 2, $msg . ': ' . $path);
        break;
}
