    ...
    public static function getFileStatus($file_path, $original_file_path = null, $redirectFile)
    {
        ...
        if (null === $original_file_path) {
            if (self::fileExists($file_path)) {
                ...
                $path_status .= "$file_path (" . \constant('_GLOSSAIRE_FC_AVAILABLE') . ') ';
            } else {
                $path_status = "<img src='$pathIcon16/0.png' >";
                $path_status .= "$file_path (" . \constant('_GLOSSAIRE_FC_NOTAVAILABLE') . ') ';
            }
        } else {
            if (self::compareFiles($file_path, $original_file_path)) {
                ...
                $path_status .= "<button class='submit' onClick='this.form.submit();'>" . \constant('_GLOSSAIRE_FC_CREATETHEFILE') . '</button>';
                $path_status .= '</form>';
            }
        }
...
switch ($op) {
    case 'copyfile':
        ...
        $msg = FileChecker::copyFile($original_file_path, $file_path) ? \constant('_GLOSSAIRE_FC_FILECOPIED') : \constant('_GLOSSAIRE_FC_FILENOTCOPIED');
