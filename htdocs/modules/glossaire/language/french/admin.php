<?php
   /**
 * Name: modinfo.php
 * Description:
 *
 * You may not change or alter any portion of this comment or credits
 * of supporting developers from this source code or any supporting source code
 * which is considered copyrighted (c) material of the original comment or credit authors.
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 *
 * @copyright The XOOPS Project http://sourceforge.net/projects/xoops/
 * @license http://www.fsf.org/copyleft/gpl.html GNU public license
 * @package : XOOPS
 * @Module : 
 * @subpackage : Menu Language
 * @since 2.5.7
 * @author Jean-Jacques DELALANDRE (jjdelalandre@orange.fr)
 * @version {version}
 * Traduction:  
 */
 
defined( 'XOOPS_ROOT_PATH' ) or die( 'Accès restreint' );

define('_AM_GLOSSAIRE_ABOUT_MAKE_DONATION', "Soumettre");
define('_AM_GLOSSAIRE_ACTIVATE', "Activer");
define('_AM_GLOSSAIRE_CAT_SHOW_BIN', "Champ à afficher");
define('_AM_GLOSSAIRE_CATEGORY_ACTIVE_DESC', "<span style='color:red;'>Si faux la catégorie ne sera pas visible dans l'interface utilisateur.</span>");
define('_AM_GLOSSAIRE_CATEGORY_ADD', "Ajouter une catégorie");
define('_AM_GLOSSAIRE_CATEGORY_BR_AFTER_TERME', "Retour à la ligne après le terme");
define('_AM_GLOSSAIRE_CATEGORY_BR_AFTER_TERME_DESC', "Oui : les champs 'term' et 'short_def' seront sur deux lignes.<br><Non>les champs 'term' et 'short_def' seront sur la même ligne.");
define('_AM_GLOSSAIRE_CATEGORY_CSS_OK', "Enregistrement du fichier CSS ok");
define('_AM_GLOSSAIRE_CATEGORY_DATE_CREATION', "Création");
define('_AM_GLOSSAIRE_CATEGORY_DATE_UPDATE', "Mise à jour");
define('_AM_GLOSSAIRE_CATEGORY_EDIT', "Modifier la catégorie");
define('_AM_GLOSSAIRE_CATEGORY_EDIT_CSS', "Edition des styles pour cette catégorie");
define('_AM_GLOSSAIRE_CATEGORY_ENTRIES', "Entrées");
define('_AM_GLOSSAIRE_CATEGORY_FOLDER_DESC', "Dossier de téléchargement des fichiers et images du glossaire dans le dossier \"uploads\"<br>Le nom du dossier ne doit pas contenir de caractères spcéciaux, accents, espaces, ...<br>Si laissé vide , le nom du dossier reprend le nom du glossaire");
define('_AM_GLOSSAIRE_CATEGORY_ID', "Identifiant");
define('_AM_GLOSSAIRE_CATEGORY_MAGNIFY_SD', "Magnifier");
define('_AM_GLOSSAIRE_CATEGORY_MAGNIFY_SD_DESC', "Met en forme la définiton courte en mettant les lettres majuscules en gras ainsi que les lettres précédées par un \"/\"<br>Valeur par défaut modifiable pour les entrées de la catégorie");
define('_AM_GLOSSAIRE_CATEGORY_TOTAL', "Total");
define('_AM_GLOSSAIRE_CATEGORY_UMLOAD_FOLDER', "Dossier des fichiers");
define('_AM_GLOSSAIRE_CLEAN_ENTRIES_FILES', 'Nettoyer les fichiers joints (%1\$s fichiers inexistants, %2\$s fichiers non référencées)');
define('_AM_GLOSSAIRE_CLEAN_ENTRIES_IMAGES', 'Nettoyer les images (%1\$s images inexistantes, %2\$s images non référencées)');
define('_AM_GLOSSAIRE_CLEAN_ENTRIES_IMAGES_OK', "Nettoyage des images : <br>%s définitions mises à jour et %s image(s) supprimées<br>pour la catégorie #%s.");
define('_AM_GLOSSAIRE_DATE_FORMAT', "Format des dates");
define('_AM_GLOSSAIRE_DATE_FORMAT_DESC', "Voir l'aide de PHP pour plus d'informations");
define('_AM_GLOSSAIRE_DELETE_FILE', "Supprimer le fichier");
define('_AM_GLOSSAIRE_DELETE_IMG', "Supprimer l'image");
define('_AM_GLOSSAIRE_DESACTIVATE', "Désactiver");
define('_AM_GLOSSAIRE_DONATION_AMOUNT', "Montant du don");
define('_AM_GLOSSAIRE_DOWNLOAD_OK', "Action terminée avec succès, le fichier se trouve dans votre dossier de téléchargement.");
define('_AM_GLOSSAIRE_ENT_BTN_ACTIONS_BOTTOM', "Position en bas des boutons de gestion");
define('_AM_GLOSSAIRE_ENT_BTN_ACTIONS_TOP', "Position en haut des boutons de gestion");
define('_AM_GLOSSAIRE_ENT_LINK', "Lien sur le terme");
define('_AM_GLOSSAIRE_ENT_ADD_LINK_IN_URLS', "Ajouter le lien dans \"voir aussi\"");
define('_AM_GLOSSAIRE_ENT_CREATOR', "Auteur");
define('_AM_GLOSSAIRE_ENT_DEFINITION', "Définition");
define('_AM_GLOSSAIRE_ENT_EMAIL', "Contact");
define('_AM_GLOSSAIRE_ENT_FILE_NAME', "Fichier joint");
define('_AM_GLOSSAIRE_ENT_HEADER', "Haut de page : Index et Barre de recherche par lettres");
define('_AM_GLOSSAIRE_ENT_ID', "ID");
define('_AM_GLOSSAIRE_ENT_IMAGE', "Image");
define('_AM_GLOSSAIRE_ENT_MAGNIFY', "Magnifier");
define('_AM_GLOSSAIRE_ENT_REFERENCE', "Référence");
define('_AM_GLOSSAIRE_ENT_SHORTDEF', "Définition courte");
define('_AM_GLOSSAIRE_ENT_URLS', "Voir aussi");
define('_AM_GLOSSAIRE_ENTRIES_UPDATE_OK', "Mise à jour effectuée");
define('_AM_GLOSSAIRE_ENTRY_ADD', "Ajouter une entrée");
define('_AM_GLOSSAIRE_ENTRY_CURRENT_FILE', "Fichier actuel");
define('_AM_GLOSSAIRE_ENTRY_DATE_CREATION', "Date de création");
define('_AM_GLOSSAIRE_ENTRY_DATE_UPDATE', "Date de mise à jour");
define('_AM_GLOSSAIRE_ENTRY_FLAG', "Drapeau");
define('_AM_GLOSSAIRE_ENTRY_ID', "Identifiant");
define('_AM_GLOSSAIRE_ENTRY_IMG_DESC', "Sélectionnez une image existante ou téléchargez une nouvelle image.<br>Laissez vide pour garder l'image existante.");
define('_AM_GLOSSAIRE_ENTRY_INITIALE', "Initiale");
define('_AM_GLOSSAIRE_ENTRY_MAGNIFY_SD', "Magnifier");
define('_AM_GLOSSAIRE_ENTRY_MAGNIFY_SD_DESC', "Met en forme la définiton courte en mettant les lettres majuscules en gras ainsi que les lettres précédées par un \"/\"");
define('_AM_GLOSSAIRE_ENTRY_UID', "UId");
define('_AM_GLOSSAIRE_ENTRY_URL1', "Url1");
define('_AM_GLOSSAIRE_ENTRY_URL2', "Url2");
define('_AM_GLOSSAIRE_EXPORT', "Export");
define('_AM_GLOSSAIRE_EXPORT_AVERTISSEMENT', "Exportation : nettoyage des fichiers.");
define('_AM_GLOSSAIRE_EXPORT_AVERTISSEMENT_DESC', "<b>Important</b> : Pour alléger l'archive l'export effectue un nettoyage des fichiers images et fichiers joints inutiles<br>qui n'ont pas été supprimés correctement lors de modifications");
define('_AM_GLOSSAIRE_EXPORTER', "Exporter");
define('_AM_GLOSSAIRE_FILE_DESC', "Une nouvelle catégorie sera générée");
define('_AM_GLOSSAIRE_FILE_NAME', "Titre du fichier");
define('_AM_GLOSSAIRE_FILE_TO_IMPORT', "Fichier à importer");
define('_AM_GLOSSAIRE_FILE_TO_IMPORT_DESC', "Le fichier doit être une archive exportée par le module 'glossaire' ou l'un de ses clones.<br>A utiliser quand le fichier est trop gros pour etre importé.<br>Copiez le fichier via FTP dans le dossier 'uploads/glossaire/imports-directs'.");
define('_AM_GLOSSAIRE_FILE_TO_LOAD', "Fichier à importer");
define('_AM_GLOSSAIRE_IMPORT_ECHEC', "Echec de l'importation<br>%s");
define('_AM_GLOSSAIRE_IMPORT_FROM_FTP', "Import direct du dossier 'mport-direct'");
define('_AM_GLOSSAIRE_IMPORT_FROM_GLOSSAIRE', "Import d'un export du module Gossaire ou d'un de ses clones");
define('_AM_GLOSSAIRE_IMPORT_FROM_LEXIKON', "Import du modulle Lexikon (Lexikon doit être installé)");
define('_AM_GLOSSAIRE_IMPORT_IN_NEW_CAT', "Importer dans une nouvelle catégorie");
define('_AM_GLOSSAIRE_IMPORT_SUCCES', "Importation Ok dans catIdSelect #%s");
define('_AM_GLOSSAIRE_INCLUDE_FILES', "inclure les fichiers joints");
define('_AM_GLOSSAIRE_INCLUDE_FILES_DESC', "Si vous n'incluez pas les fichiers joints l'archives sera plus légère.");
define('_AM_GLOSSAIRE_INCLUDE_IMG', "Inclure les images");
define('_AM_GLOSSAIRE_INCLUDE_IMG_DESC', "Si les images ne sont pas incluses l'archive sera plus légère");
define('_AM_GLOSSAIRE_MAINTAINEDBY', " est maintenu par ");
define('_AM_GLOSSAIRE_NB_COLUMNS_INDEX', "Nb colonnes de l'index");
define('_AM_GLOSSAIRE_NB_COLUMNS_INDEX_DESC', "Nombre de colonnes pour l'affichage de l'index pour la page courante.<br> > 0 : nombre de colonnes d'affichage de l'index (defaut = 1)<br>&nbsp;0 : Affichage en continu séparé par ' | '<br>-1 : pas d'affichage de l'index");
define('_AM_GLOSSAIRE_NO_CATEGORIES1', "Il n'y a aucune categorie de crée.<br>Vous devez d'abord créer une catégories avant d'ajouter des définitions.");
define('_AM_GLOSSAIRE_NO_CATEGORIES2', "Il n'y a aucune categorie à exporter.");
define('_AM_GLOSSAIRE_PERM_CLONE', "Cloner le module");
define('_AM_GLOSSAIRE_PERM_DESC', "Gestion des permissions");
define('_AM_GLOSSAIRE_PERM_EXPORT', "Exporter un glossaire");
define('_AM_GLOSSAIRE_PERM_GLOBAL_AC', "Permissions globales");
define('_AM_GLOSSAIRE_PERM_IMPORT', "Importer un glossaire");
define('_AM_GLOSSAIRE_PERM_MANCATS', "Gérer les catégories");
define('_AM_GLOSSAIRE_PERM_PERMS', "Gérer les permissions");
define('_AM_GLOSSAIRE_PERM_VIEW_CATS', "Voir les catégories");
define('_AM_GLOSSAIRE_PERMISSIONS', "Permissions");
define('_AM_GLOSSAIRE_PERM_APPROVE_ENTRIES', "Approuver les entrées");
define('_AM_GLOSSAIRE_PERM_SUBMIT_ENTRIES', "Soumettre des entrées");
define('_AM_GLOSSAIRE_REPLACE_AROBASE', "Remplacer l'arobase");
define('_AM_GLOSSAIRE_REPLACE_AROBASE_DESC', "Permet de bloquer les accès directs au courriels sur les pages.<br>exemple : [@]");
define('_AM_GLOSSAIRE_SELECT_CATEGORY_DESC', "Sélectionnez une catégorie de destintion pour ces nouvelles définitions.");
define('_AM_GLOSSAIRE_SELECT_LEX_CATEGORY', "Catégories de Lexikon");
define('_AM_GLOSSAIRE_SELECT_LEX_CATEGORY_DESC', "Sélectionez la catégorie du module Lexikon à importer");
define('_AM_GLOSSAIRE_STATISTICS', "Statistiques");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_CREATOR', "Auteur");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_DATE_UPDATE', "Date de mise à jour");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_DEFINITION', "Définition");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_EMAIL', "Courriel");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_FILE_NAME', "Fichier joint");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_FILES_JOINS', "Fichiers joints");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_INDEX_DIV', "Index des termes en liste");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_INDEX_TABLE', "<hr>Index des termes en colonnes");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_REFERENCE', "Références");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_SHORTDEF', "Définition courte");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_TERM', "<hr>Terme");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_URLS', "URL à visiter");
define('_AM_GLOSSAIRE_STYLES_GLS_LETTER_DEFAULT', "Alphabet : Lettres par défaut");
define('_AM_GLOSSAIRE_STYLES_GLS_LETTER_EMPTY', "Alphabet : Lettres absentes");
define('_AM_GLOSSAIRE_STYLES_GLS_LETTER_EXIST', "Alphabet : Lettres existantes");
define('_AM_GLOSSAIRE_STYLES_GLS_LETTER_SELECTED', "Alphabet : Lettre sélectionée");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_DATES', "Dates");
define('_AM_GLOSSAIRE_STYLES_GLS_ENT_COUNTER', "Compteur");
define('_AM_GLOSSAIRE_STYLES_GLS_LETTER_SELECED', "Lettre sélectionnée");
define('_AM_GLOSSAIRE_STYLES_GLS_STYLE_SHEET', "Feuille de style");
define('_AM_GLOSSAIRE_STYLES_INIT_STYLE_SHEET', "Initialize la feuille de style");
define('_AM_GLOSSAIRE_FORM_SURE_INIT_CSS', "Confirmer l'initialisation de la feuille de style de [%s] %s");
define('_AM_GLOSSAIRE_FORM_INIT_CSS_OK', "La feuille de style de [%s] %s a été initialisée avec succès.");

define('_AM_GLOSSAIRE_SUBMIT_AND_ADDNEW', "Soumettre et ajouter");
define('_AM_GLOSSAIRE_THEREARE_CATEGORIES', "Il y a <span class='bold'>%s</span> catégories dans la base de données");
define('_AM_GLOSSAIRE_THEREARE_ENTRIES', "Il y a <span class='bold'>%s</span> entrées dans la base de données");
define('_AM_GLOSSAIRE_THEREARENT_CATEGORIES', "Il n'y a pas de catégories");
define('_AM_GLOSSAIRE_THEREARENT_ENTRIES', "Il n'y a pas d'entrées");
define('_AM_GLOSSAIRE_TOTAL', "Total");
define('_AM_GLOSSAIRE_WEIGHT_UPDATE', "Le poids a été mis à jour");
define('_AM_GLOSSAIRE_CATEGORY_ACTIVE', "Active");
define('_AM_GLOSSAIRE_CATEGORY_COLOR_SET', "Jeu de couleurs");
define('_AM_GLOSSAIRE_CATEGORY_FOLDER', "Dossier");
define('_AM_GLOSSAIRE_CATEGORY_LOGO', "Logo");
define('_AM_GLOSSAIRE_CATEGORY_NAME', "Nom");
define('_AM_GLOSSAIRE_CATEGORY_WEIGHT', "Poids");
define('_AM_GLOSSAIRE_DOWN', "Descendre");
define('_AM_GLOSSAIRE_EDIT_CSS', "Edit CSS");
define('_AM_GLOSSAIRE_ENTRIES', "Entrées");
define('_AM_GLOSSAIRE_ENTRY_EDIT', "Modifier l'entrée");
define('_AM_GLOSSAIRE_ENTRY_EMAIL', "Courriel");
define('_AM_GLOSSAIRE_ENTRY_EMAIL_DESC', "Contact par courriel.");
define('_AM_GLOSSAIRE_ENTRY_FILE', "Fichier");
define('_AM_GLOSSAIRE_ENTRY_IMAGE_UPLOADS', "Image in %s :");
define('_AM_GLOSSAIRE_ENTRY_IMG_DESC2', "Sélectionnez une image dans votre environnement ou Laissez vide.");
define('_AM_GLOSSAIRE_ENTRY_REFERENCES_DESC', "Sources, Adresse, téléphone et toute information utile");
define('_AM_GLOSSAIRE_ENTRY_URLS', "Sites de référence");
define('_AM_GLOSSAIRE_ENTRY_URLS_DESC', "Entrer une URL par ligne sans séparateur.Chaque ligne peut etre composée du titre et de l'url séparée par un '|' sans espace.<br>Exemple : <br>Xoops France<b> | </b>https://www.frxoops.org/");
define('_AM_GLOSSAIRE_FIRST', "Premier");
define('_AM_GLOSSAIRE_FORM_ACTIONS', "Actions");
define('_AM_GLOSSAIRE_FORM_DELETE_OK', "Supprimé avec succès");
define('_AM_GLOSSAIRE_FORM_SURE_DELETE', "Êtes-vous sûr de supprimer : <b><span style='color : Red;'>[%s] %s</span></b>");
define('_AM_GLOSSAIRE_LAST', "Dernier");
define('_AM_GLOSSAIRE_RAZ_COUNTERS', "RAZ des compteurs");
define('_AM_GLOSSAIRE_UP', "Monter");
define('_AM_GLOSSAIRE_ADD_CATEGORY', "Ajouter une nouvelle catégorie");
define('_AM_GLOSSAIRE_ADD_ENTRY', "Ajouter une nouvelle entrée");
define('_AM_GLOSSAIRE_CATEGORY_DESCRIPTION', "Description");
define('_AM_GLOSSAIRE_ENT_COUNTER', "Compteur de visites");
define('_AM_GLOSSAIRE_ENT_DATE_CREATION', "Date de création");
define('_AM_GLOSSAIRE_ENT_DATE_UPDATE', "Date de mise à jour");
define('_AM_GLOSSAIRE_ENTRY_CAT_ID', "Identifiant du chat");
define('_AM_GLOSSAIRE_ENTRY_DEFINITION', "Définition");
define('_AM_GLOSSAIRE_ENTRY_REFERENCES', "Références");
define('_AM_GLOSSAIRE_ENTRY_SHORTDEF', "Définition courte");
define('_AM_GLOSSAIRE_ENTRY_STATUS', "Statut");
define('_AM_GLOSSAIRE_ENTRY_TERM', "Terme");
define('_AM_GLOSSAIRE_FORM_UPLOAD_SIZE', "Taille maximale du fichier : ");
define('_AM_GLOSSAIRE_FORM_UPLOAD_SIZE_MB', "Mo");
define('_AM_GLOSSAIRE_IMPORTER', "Importer");
define('_AM_GLOSSAIRE_LIST_ENTRIES', "Liste des entrées");
define('_AM_GLOSSAIRE_CATEGORY', "Catégorie");
define('_AM_GLOSSAIRE_CATEGORY_STATUS_APPROVED', "Approuvé");
define('_AM_GLOSSAIRE_CATEGORY_STATUS_INATIF', "Inactif");
define('_AM_GLOSSAIRE_CATEGORY_STATUS_PROPOSITION', "Proposé");
define('_AM_GLOSSAIRE_ENTRY_IMAGE', "Image");
define('_AM_GLOSSAIRE_FILE_UPLOADSIZE', "Taile maximum des fichiers %s mo");
define('_AM_GLOSSAIRE_LIST_CATEGORIES', "Liste des Catégories");
define('_AM_GLOSSAIRE_ENTRY_COUNTER', "Compteur");
define('_AM_GLOSSAIRE_FORM_OK', "Sauvegardé avec succès");
define('_AM_GLOSSAIRE_CREATOR', "Auteur");
define('_AM_GLOSSAIRE_NO_PERMISSIONS_SET', "Vous n'avez pas les permissions pour accéder à cette fonctionalité");

?>
