 ...
 * @return string
 */
function glossaire_block_addCatSelect($cats)
{
    $cat_sql = '(';
    if (\is_array($cats)) {
 ...
 */
function glossaireGetMyItemIds($permtype, $dirname)
{
    global $xoopsUser;
    static $permissions = [];
    ...
    $glossaireModule = $moduleHandler->getByDirname($dirname);
    $groups = \is_object($xoopsUser) ? $xoopsUser->getGroups() : \XOOPS_GROUP_ANONYMOUS;
    $grouppermHandler = \xoops_getHandler('groupperm');
    $itemIds = $grouppermHandler->getItemIds($permtype, $groups, $glossaireModule->getVar('mid'));
    return $itemIds;
}

/**
 * Get the number of entries from the sub categories of a category or sub topics of or topic
 * @param $mytree
 * @param $entries
 * @param $entries
 * @param $cid
 * @return int
 */
//JJDai : ajout du prifix tbl_ au 2eme parametre de la fonction 
//        pour eviter un doublon dans le cas ou une table s'appelerai "entries"       
function glossaireNumbersOfEntries($mytree, $tbl_entries, $entries, $cid)
{
    $count = 0;
    if(\in_array($cid, $tbl_entries)) {
        $child = $mytree->getAllChild($cid);
        foreach (\array_keys($entries) as $i) {
            if ($entries[$i]->getVar('ent_id') == $cid){
                $count++;
            }
            foreach (\array_keys($child) as $j) {
                if ($entries[$i]->getVar('ent_id') == $j){
                    $count++;
                }
            }
        }
...
function glossaireMetaKeywords($content)
{
...
function glossaireMetaDescription($content)
{
    global $xoopsTpl, $xoTheme;
    $myts = MyTextSanitizer::getInstance();
    $content = $myts->undoHtmlSpecialChars($myts->displayTarea($content));
...
function glossaire_RewriteUrl($module, $array, $type = 'content')
{
    ...
    if (isset($array['topic_alias']) && $array['topic_alias']) {
        ...
        $topic_name = glossaire_Filter(xoops_getModuleOption('static_name', $module));
    }
